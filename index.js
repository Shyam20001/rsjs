...
//
// NOTE: This file is the "speedy" drop-in variant:
//  - Offloads heavy per-request work from the TSFN callback using setImmediate,
//    so the native enqueue returns immediately (dramatic reduction in Rust-side wait_us).
//  - Preserves routing, query, header, cookie, and response semantics.
//  - Preserves shutdown semantics by incrementing `pending` synchronously (like original).
//  - Minimal other changes; behavior remains compatible with existing user handlers.
...
function createApp() {
  ...
  // --- optimized attachNative: offload heavy work from TSFN callback ---
  function attachNative() {
    ...
    registerJsCallback((_, rawParts) => {
      // Keep the TSFN callback extremely fast.
      // Capture minimum info synchronously, increment pending (preserve semantics),
      // then offload heavy parsing/routing to setImmediate.
      ...
      // Synchronously capture a small object and increment pending to preserve shutdown semantics.
      const captured = {
        reqIdRaw, pathRaw, methodRaw, rawQueryRaw,
        headersJsonRaw, bodyRaw, peerAddrRaw, cookieHeaderRaw
      };

      // PRESERVE original semantics: increment pending synchronously (so shutdown logic remains correct).
      pending++;

      // Offload heavy synchronous work to the next tick so TSFN returns immediately.
      setImmediate(() => {
        try {
          const {
            reqIdRaw, pathRaw, methodRaw, rawQueryRaw,
          ...
          // If something goes wrong during async processing, attempt to respond 500
          try {
            const id = captured && captured.reqIdRaw != null ? String(captured.reqIdRaw) : '';
            if (id) respond(id, JSON.stringify({ status: 500, headers: { 'Content-Type': 'text/plain' }, body: 'Internal Server Error' }));
          } catch (_) {}
          try { if (pending > 0) pending--; } catch (_) {}
        }
      });

      return ''; // napi callback string return (fast)
    });
  }
